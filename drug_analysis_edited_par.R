...
direc <- '/afs/.ir.stanford.edu/users/s/a/sasidhar/naccdataset/greicius_project/'
#accept arguments
args <- commandArgs(TRUE)
drugNum <- args[1]
drugNum <- as.numeric(drugNum)

#parameters
#outputfilename <- paste0("drug_coxph_summ_",drugNum,".txt")
#outputfilename <- paste0("/afs/.ir.stanford.edu/users/s/a/sasidhar/naccdataset/greicius_project/drug_coxph_summ_",drugNum,".txt")
#Subset of patient
...
data <- read.csv(paste0(direc,"altmann06122013.csv"), as.is=T)
data.df <- data.frame(data)

#prepared data
xtab.fname <- paste0(direc,"drug_conversion.RData")
#xtab.fname <- "/mnt/mapricot/musk2/home/altmann/data/NACC/RData/drug_conversion.RData"

#Read in each of the drugs#
drug_list <- read.csv(paste0(direc,"sorted_unique_drugs_table.csv"), header = TRUE, sep = ",")
...
if (patientSubset == 1) {
	...
	#LOAD NUMDRUGS
	load(paste0(direc,"NUMDRUGSHC.RData"))
}
	...
	#LOAD NUMDRUGS
	load(paste0(direc,"NUMDRUGSMCI.RData"))
}
	...
	#LOAD NUMDRUGS
	load(paste0(direc,"NUMDRUGS.RData"))
}
...
#message stats alone-SASI
row_of_interest <- coef(summary(coxph_val))
message(row_of_interest[1,5])
message(row_of_interest[1,1])
